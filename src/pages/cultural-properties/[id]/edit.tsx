...
export default function EditCulturalPropertyPage() {
  ...
  const fetchProperty = useCallback(async () => {
    ...
    try {
      ...
      // タグを設定（✅ Phase 3-3追加）
      if (data.tags) {
        setSelectedTags(data.tags.map(t => ({ id: t.id, name: t.name, description: t.description })))
      }
    } catch (error) {
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 sm:py-8">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white shadow-sm rounded-xl p-5 sm:p-6 mb-5 sm:mb-6 border border-gray-100">
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowLinkMovieModal(true)} className="inline-flex items-center px-3 py-1.5 text-sm font-medium text-purple-700 bg-purple-50 rounded-lg hover:bg-purple-100 transition-colors cursor-pointer">
                ...
                <button type="button" onClick={() => setShowAddMovieForm(true)} className="inline-flex items-center px-3 py-1.5 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700 transition-colors cursor-pointer">
                ...
                {linkedMovies.map((movie) => (
                  <div key={movie.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex items-center space-x-2 ml-3">
                      ...
                      <button type="button" onClick={() => handleUnlinkMovie(movie)} className="p-2 text-gray-500 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors cursor-pointer" title="紐付け解除">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                      </button>
                      <button type="button" onClick={() => handleDeleteMovie(movie)} className="p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer" title="削除">
            ...
            {showAddMovieForm && (
              <div className="mt-5 p-4 bg-purple-50 rounded-xl border-2 border-purple-200">
                ...
                <div className="flex justify-end gap-2 mt-4">
                  <button type="button" onClick={() => { setShowAddMovieForm(false); setNewMovieData({ url: '', title: '', note: '' }); setMovieErrors({}) }} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer">キャンセル</button>
                  <button type="button" onClick={handleAddMovie} disabled={isAddingMovie} className={`px-4 py-2 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700 cursor-pointer ${isAddingMovie ? 'opacity-50 cursor-not-allowed' : ''}`}>{isAddingMovie ? '追加中...' : '追加する'}</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pb-8">
            ...
            <button type="submit" disabled={isSaving} className={`w-full sm:w-auto px-8 py-3.5 text-base font-semibold text-white bg-blue-600 border-2 border-transparent rounded-xl hover:bg-blue-700 transition-all shadow-md hover:shadow-lg flex items-center justify-center cursor-pointer ${isSaving ? 'opacity-50 cursor-not-allowed' : ''}`}>
              ...
              <div className="relative bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full">
                <div className="bg-white px-4 pt-5 pb-4 sm:p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button type="button" onClick={() => setShowLinkMovieModal(false)} className="p-2 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100 cursor-pointer">
                  ...
                  {moviesLoading ? (
                    ...
                    <div className="space-y-2 max-h-80 overflow-y-auto">
                      {unlinkedMovies.map((movie) => (
                        <button key={movie.id} type="button" onClick={() => handleLinkMovie(movie)} className="w-full flex items-center p-3 bg-gray-50 rounded-xl hover:bg-purple-50 hover:border-purple-200 border-2 border-transparent transition-colors text-left cursor-pointer">
                          <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center flex-shrink-0">
                            <svg className="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                          </div>
