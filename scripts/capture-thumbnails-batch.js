...
async function main() {
  ...
  if (fs.existsSync(moviesJsonPath)) {
    ...
    console.error(
      '映画データが見つかりません。先に npm run generate-luma-list を実行してください。',
    )
    return
  }
  ...
  for (const movie of lumaMovies) {
    ...
    if (thumbnailExists(movie.id)) {
      console.log(
        `Movie ID: ${movie.id} (${movie.title}) のサムネイルは既に存在するためスキップします。`,
      )
      skippedCount++
      continue
    }
  ...
  console.log(
    `処理完了: ${processedCount}件のサムネイルを生成、${skippedCount}件をスキップしました。`,
  )
